...
public class DistExecutor {
    ...
    /**
     * Run the callable in the supplier only on the specified {@link Dist}. This method is NOT
     * sided-safe and special care needs to be taken in code using this method that implicit class
     ...
     * @param toRun A supplier of the callable to run (Supplier wrapper to ensure classloading only on
     *              the appropriate dist)
     * @param <T>   The return type from the callable
     * @return The callable's result
     * @deprecated use {@link #safeCallWhenOn(Dist, Supplier)} instead. This remains for advanced use
     *             cases.
     */
    @Deprecated
    ...
    /**
     * Runs the supplied Runnable on the speicified side. Same warnings apply as {@link
     ...
     *
     * <p>
     * This method can cause unexpected ClassNotFoundException problems in common scenarios.
     * Understand the pitfalls of the way the class verifier works to load classes before using this.
     *
     * <p>
     ...
     */
    @Deprecated
    public static <T> T runForDist(
                                   Supplier<Supplier<T>> clientTarget, Supplier<Supplier<T>> serverTarget) {
        return unsafeRunForDist(clientTarget, serverTarget);
    }

    /**
     * Unsafe version of {@link #safeRunForDist(Supplier, Supplier)}. Use only when you know what
     * you're doing and understand why the verifier can cause unexpected ClassNotFoundException
     * crashes even when code is apparently not sided. Ensure you test both sides fully to be
     ...
     * @return The returned instance
     */
    public static <T> T safeRunForDist(
                                       Supplier<SafeSupplier<T>> clientTarget, Supplier<SafeSupplier<T>> serverTarget) {
    ...
    private static final void validateSafeReferent(
        ...
        for (Class<?> cl = setter.getClass(); cl != null; cl = cl.getSuperclass()) {
            try {
                ...
                if (!(replacement instanceof SerializedLambda l)) break; // custom interface implementation
    ...
    /**
     * A safe referent. This will assert that it is being called via a separated class method
     * reference. This will avoid the common pitfalls of {@link #callWhenOn(Dist, Supplier)} above.
     *
     * <p>
     * SafeReferents assert that they are defined as a separate method outside the scope of the
     * calling class.
     *
     * <p>
     * <strong>Implementations need to be defined in a separate class to the referring site, with
     * appropriate visibility to be accessible at the callsite (generally, avoid private
     * methods).</strong>
     *
     * <p>
     * Valid:<br>
